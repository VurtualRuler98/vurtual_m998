...
_veh addAction ["Passenger mode",{[(_this select 0),false,false] call vurtual_m998_fnc_cargobed;},[],1.5,false,true,"","(alive _target) && (driver _target == _this) && (count getVehicleCargo _target)==0 && (vehicleCargoEnabled _target)",15,false,"tailgate"];
_veh addAction ["VIV Cargo mode",{[(_this select 0),true,false] call vurtual_m998_fnc_cargobed;},[],1.5,false,true,"","(alive _target) && (driver _target == _this) && !(vehicleCargoEnabled _target)",15,false,"tailgate"];
true
